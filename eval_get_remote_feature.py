...
from sample4geo.dataset.mvcv import MVCVDatasetEval,MVCVDatasetEvalRemote, MVCVDatasetTrainComplex2
from sample4geo.transforms import get_transforms_train_complex, get_transforms_val
from sample4geo.utils import setup_system, Logger
from sample4geo.trainer_mvcv import trainComplex3
from sample4geo.evaluate.cvusa_and_cvact import evaluate_and_save, calc_sim
from sample4geo.loss import InfoNCE
from sample4geo.model import MVCV4Geo,MVCV4GeoSym,DINO4Geo,MVCV4GeoComplex2
...
class Configuration:
    ...
    # Model
    model: str = "BLIP-2-ViT-L--Remote-CLIP--Token-Pool--extra-sat_0.5_两个跨视角loss_1"
...
if __name__ == "__main__":
    ...
    model_path = "{}/{}/{}".format(
        config.model_path, config.model, time.strftime(r"%Y-%m-%d_%H-%M-%S")
    )
    ...
    # model = DINO4Geo(img_size=config.img_size,vit_size="giant")
    model = MVCV4GeoComplex2(img_size=config.img_size)
    # model = MVCV4GeoSym(img_size=config.img_size)
    ...
    # Transforms
    sat_transforms_train1, sat_transforms_train2, ground_transforms_train1, ground_transforms_train2 = get_transforms_train_complex(
        image_size_sat,
        img_size_ground,
        mean=mean,
        std=std,
    ...
    # Train
    train_dataset = MVCVDatasetTrainComplex2(
    ...
    reference_dataset_test = MVCVDatasetEvalRemote(
        folder_path="/home/***/codes/MultiViewGeo/zhengzhou_700",
        img_type="reference",
        transform=sat_transforms_val,
    )
    ...
    query_dataset_test = MVCVDatasetEval(
        data_folder=config.data_folder,
        split="test",
        img_type="query1",
    ...
    # Query Ground Images Test
    query_dataset_test2 = MVCVDatasetEval(
        data_folder=config.data_folder,
        split="test",
        img_type="query2",
        transforms=ground_transforms_val,
    ...
    query_dataloader_test2 = DataLoader(
        query_dataset_test2,
        batch_size=config.batch_size_eval,
        num_workers=config.num_workers,
        shuffle=False,
    ...
    if config.sim_sample:
        ...
        query_dataset_train = MVCVDatasetEval(
            data_folder=config.data_folder,
            split="train",
            img_type="query1",
            transforms=ground_transforms_val,
        ...
        reference_dataset_train = MVCVDatasetEval(
            data_folder=config.data_folder,
            split="train",
            img_type="reference",
    ...
    if config.scheduler == "polynomial":
        print(
            "\nScheduler: polynomial - max LR: {} - end LR: {}".format(
                config.lr, config.lr_end
            )
        )
    ...
    print(
        "Warmup Epochs: {} - Warmup Steps: {}".format(
            str(config.warmup_epochs).ljust(2), warmup_steps
        )
    )
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        print(
            "Epoch: {}, Train Loss = {:.3f}, Lr = {:.6f}".format(
        ...
        # evaluate
        if (
            epoch % config.eval_every_n_epoch == 0 and epoch != 0
        ) or epoch == config.epochs:

            print("\n{}[{}]{}".format(30 * "-", "Evaluate", 30 * "-"))

            r1_cross_test,r1_cross_test2,r1_same = evaluate(
                config=config,
                model=model,
                reference_dataloader=reference_dataloader_test,
                query_dataloader=query_dataloader_test,
                query_dataloader2=query_dataloader_test2,
                ranks=[1, 5, 10],
                step_size=1000,
                cleanup=True,
            )
            ...
            if r1_cross_test > best_score:

                best_score = r1_cross_test

                if torch.cuda.device_count() > 1 and len(config.gpu_ids) > 1:
                    torch.save(
                        model.module.state_dict(),
                        "{}/weights_e{}_{:.4f}_{:.4f}_{:.4f}.pth".format(model_path, epoch, r1_cross_test, r1_cross_test2, r1_same),
                    )
                else:
                    torch.save(
                        model.state_dict(),
