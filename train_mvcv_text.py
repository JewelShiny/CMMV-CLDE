...
from sample4geo.dataset.mvcv import MVCVDatasetEvalTextLLM, MVCVDatasetTrainTextLLM
from sample4geo.transforms import get_transforms_train, get_transforms_val
from sample4geo.utils import setup_system, Logger
from sample4geo.trainer_mvcv import trainText
from sample4geo.evaluate.cvusa_and_cvact import evaluate_text, calc_sim
from sample4geo.loss import InfoNCE
from sample4geo.model import MVCV4Geo,MVCV4GeoSym,DINO4Geo,MVCV4GeoTextLLM
...
class Configuration:
    ...
    seed = 104
    # Model
    model: str = f"BLIP-2-ViT-L--Remote-CLIP--40epoch--Text--LLM--first-{seed}"
...
if __name__ == "__main__":
    ...
    model = MVCV4GeoTextLLM(img_size=config.img_size)
    ...
    # Train
    train_dataset = MVCVDatasetTrainTextLLM(
        ...
        multi_ground=config.multi_ground
    )
    ...
    reference_dataset_test = MVCVDatasetEvalTextLLM(
        data_folder=config.data_folder,
        split="test",
        img_type="reference",
    ...
    query_dataset_test = MVCVDatasetEvalTextLLM(
        data_folder=config.data_folder,
        split="test",
        img_type="query",
    ...
    if config.sim_sample:
        ...
        query_dataset_train = MVCVDatasetEvalTextLLM(
            data_folder=config.data_folder,
            split="train",
            img_type="query1",
            transforms=ground_transforms_val,
        ...
        reference_dataset_train = MVCVDatasetEvalTextLLM(
            data_folder=config.data_folder,
            split="train",
            img_type="reference",
            transforms=sat_transforms_val,
            ...
            if r1_cross_test > best_score:
                ...
                if torch.cuda.device_count() > 1 and len(config.gpu_ids) > 1:
                    torch.save(
                        ...
                        "{}/weights_e{}_{:.4f}.pth".format(model_path, epoch, r1_cross_test),
                    )
                else:
                    torch.save(
                        model.state_dict(),
