...
class Blip2QformerCirAlignPrompt(Blip2Base):
    ...
    def __init__(
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder(
            vit_model, img_size, drop_path_rate, use_grad_checkpoint, vit_precision,self.pool_type
        )
        # if freeze_vit:
        if False:
    ...
    def encode_images_and_text(self,image,text):
        ###============== reference text fusion ===================###
        # reference image feature  
        image_embeds = self.ln_vision(self.visual_encoder(image))
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(
        ...
        # print("text_tokens")
        # # print(text_tokens)
        # print("text_tokens.attention_mask.shape")
        # print(text_tokens.attention_mask.shape)
        attention_mask = torch.cat([query_atts, text_tokens.attention_mask], dim=1)
        fusion_output = self.Qformer.bert(
            text_tokens.input_ids,
        ...
        # print("fusion_output.last_hidden_state")
        # print(fusion_output.last_hidden_state) #torch.Size([64, 64, 768])
        # print(fusion_output.last_hidden_state.shape)
        # print("fusion_output.last_hidden_state[:, : query_tokens.size(1), :]")
        # print(fusion_output.last_hidden_state[:, : query_tokens.size(1), :]) #torch.Size([64, 32, 768])
        # print(fusion_output.last_hidden_state[:, : query_tokens.size(1), :].shape)

        text_output = self.Qformer.bert(
            text_tokens.input_ids, #torch.Size([64, 32])
            query_embeds=fusion_output.last_hidden_state[:, : query_tokens.size(1), :],
            attention_mask=attention_mask,
            return_dict=True,
        )
        ...
        # print("text_output")
        # print(text_output)
        # print("text_output.last_hidden_state")
        # print(text_output.last_hidden_state)
        # print("text_output.last_hidden_state.shape")
        # print(text_output.last_hidden_state.shape) #torch.Size([64, 64, 768])
        fusion_feats = F.normalize(
            text_output.last_hidden_state[:, 32, :], dim=-1
        )
        return fusion_feats
    def encode_images(self, image):
        target_visual_feats = self.visual_encoder.forward_features(image) #target_visual_feats torch.Size([8, 1024])
        # print("target_visual_feats",target_visual_feats.shape)
        
        # target_embeds = self.ln_vision(target_visual_feats)
        # print("target_embeds",target_embeds.shape)
        
        return target_visual_feats

        # target_atts = torch.ones(target_embeds.size()[:-1], dtype=torch.long).to(
        #     image.device
        # )

        # query_tokens = self.query_tokens.expand(target_embeds.shape[0], -1, -1)
        ...
        # # )
        # target_feats = F.normalize(
        #     target_output.last_hidden_state, dim=-1
        # )
        # print("target_feats",target_feats.shape)
        # target_feats_proj = F.normalize(
        #     self.vision_proj(target_output.last_hidden_state), dim=-1
        # )
        # print("target_feats_proj",target_feats_proj.shape)
