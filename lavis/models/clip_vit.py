...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, use_grad_checkpointing: bool,pool_type: str):
        super().__init__()
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        assert self.pool_type in ('tok', 'avg', 'none')

        # avg pool
        if self.pool_type =="avg":
            t = x[:, 1:, :]
            return self.ln_final(t.mean(1))
        else:
            # cls pool
            t = x[:, 0, :]
            return self.ln_final(t)
...
def create_clip_vit_L(img_size=224,use_checkpoint=False,precision="fp16",pool_type="tok"):
